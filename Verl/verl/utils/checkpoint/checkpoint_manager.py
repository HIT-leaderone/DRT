...
def write_checkpoint_tracker(root_path: str, iteration: int) -> str:
    """Atomically update the latest checkpoint tracker file."""
    tracker_file = get_checkpoint_tracker_filename(root_path)
...
def is_async_checkpoint_enabled(checkpoint_config) -> bool:
    """Return whether async checkpoint save is enabled for the given config."""
    if checkpoint_config is None:
        return False

    async_save = getattr(checkpoint_config, "async_save", None)
    if async_save is not None:
        return bool(async_save)

    try:
        if "async_save" in checkpoint_config:
...
def copy_checkpoint_path_to_hdfs(local_path: str, hdfs_path: str) -> bool:
    """Copy a local checkpoint file to HDFS."""
    if local_path is None or hdfs_path is None or not os.path.exists(local_path):
        return False
    ...
    if hdfs_parent:
        hdfs_io.makedirs(hdfs_parent, exist_ok=True)
    return hdfs_io.copy(src=local_path, dst=hdfs_path)


def cleanup_old_local_checkpoint_dirs(root_path: str, keep_steps: set[int] | None = None) -> list[str]:
    """Remove local global_step directories except the explicitly kept steps."""
    ...
    for entry in sorted(os.listdir(root_path)):
        match = re.fullmatch(r"global_step_(\d+)", entry)
        if match is None:
            continue
        ...
        local_path = os.path.join(root_path, entry)
        if not os.path.isdir(local_path):
            continue

        print(f"Checkpoint manager remove old local checkpoint dir: {os.path.abspath(local_path)}")
