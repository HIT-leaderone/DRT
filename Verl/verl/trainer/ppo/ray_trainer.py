...
from verl.utils.checkpoint.checkpoint_manager import (
    cleanup_old_local_checkpoint_dirs,
    copy_checkpoint_path_to_hdfs,
    find_latest_ckpt_path,
    get_checkpoint_tracker_filename,
    is_async_checkpoint_enabled,
...
async def submit_auto_eval_task(local_path: str, hdfs_path: str, auto_eval_config: dict[str, Any], global_steps: str):
    """Submit an automatic evaluation task after the checkpoint appears on HDFS."""
    from .launch_eval_task import launch_eval_tasks

    print(f"Auto-eval start, local_path:{local_path}, hdfs_path:{hdfs_path}")

    hdfs_files = []
    for root, _, files in os.walk(local_path):
        for file in files:
            if file != "args.json":
                hdfs_files.append(os.path.join(hdfs_path, os.path.relpath(os.path.join(root, file), local_path)))
    print("=== HDFS files to be uploaded ===")
    print("\n".join(hdfs_files))

    while True:
        is_ready = True
        for hdfs_file in hdfs_files:
            cmd_str = f"hdfs dfs -test -e {hdfs_file}"
            exit_code = os.system(cmd_str)
            if exit_code != 0:
                is_ready = False
                print(f"File {hdfs_file} is not ready!")
                break
        if is_ready:
            break
    ...
    print("Files ready!!!")
    tasks = auto_eval_config.get("tasks", [])
    print(f"Start to launch auto-eval, tasks:{tasks}")
    for task_type in tasks:
        launch_eval_tasks(
            auto_eval_config.get("prompt_type", ""),
            auto_eval_config.get("model_type", ""),
            task_type,
            hdfs_path=hdfs_path,
            pool=auto_eval_config.get("pool"),
            iter=global_steps,
            retry=10,
        )
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.auto_eval_tasks = []
        self.eval_loop = asyncio.new_event_loop()
        self.eval_thread = Thread(target=self.eval_loop.run_forever, daemon=True)
    ...
    def _save_checkpoint(self):
        ...
        async_save_enabled = is_async_checkpoint_enabled(self.config.actor_rollout_ref.actor.checkpoint)
        ...
        if self.config.trainer.default_hdfs_dir is not None and not async_save_enabled:
        ...
        if self.config.trainer.default_hdfs_dir is not None:
            copy_checkpoint_path_to_hdfs(
                ...
                os.path.join(self.config.trainer.default_hdfs_dir, f"global_step_{self.global_steps}", "data.pt"),
            )
        ...
        local_latest_checkpointed_iteration = write_checkpoint_tracker(
            self.config.trainer.default_local_dir, self.global_steps
        )
        if self.config.trainer.default_hdfs_dir is not None:
            copy_checkpoint_path_to_hdfs(
                local_latest_checkpointed_iteration,
                get_checkpoint_tracker_filename(self.config.trainer.default_hdfs_dir),
            )
            cleanup_old_local_checkpoint_dirs(self.config.trainer.default_local_dir, keep_steps={self.global_steps})

        print("Run auto-eval ray-worker!!!")
        auto_eval_config = self.config.trainer.get("auto_eval", {})
        if auto_eval_config.get("enabled", False) and actor_remote_path is not None:
            actor_local_path_hf = self.config.actor_rollout_ref.model.get("path", "")
            actor_remote_path_hf = os.path.join(actor_remote_path, "huggingface")
            print("Before Auto-eval:", auto_eval_config)
            task = asyncio.run_coroutine_threadsafe(
                submit_auto_eval_task(actor_local_path_hf, actor_remote_path_hf, auto_eval_config, str(self.global_steps)),
                self.eval_loop,
            )
            self.auto_eval_tasks.append(task)
        else:
            print("Auto-eval skipped.")
            print("auto_eval_config:", auto_eval_config)
            print("actor_remote_path:", actor_remote_path)
    ...
    def _load_checkpoint(self):
        ...
        # load from hdfs
        if self.config.trainer.default_local_dir is None and self.config.trainer.default_hdfs_dir is not None:
